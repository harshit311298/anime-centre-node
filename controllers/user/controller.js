...
module.exports = {
    ...
    signup: async (req, res, next) => {
        try {
            if (!req.body.phoneNumber) {
                return response(res, statusCode.data.BAD_REQUEST, {}, "Please provide phone number.")
            }
            if (!req.body.email) {
                return response(res, statusCode.data.BAD_REQUEST, {}, "Please provide email.")
            }
            let userFind = await service.findUser({ $or: [{ phoneNumber: req.body.phoneNumber }, { email: req.body.email }], userType: userTypeEnum.data.USER, status: { $ne: statusEnum.data.DELETED } })
            if (userFind) {
            ...
            let htmlData=mailtemplates.otp_verification.otp_verification(data)
            let sendOtp = await commonFunction.adminSendMail(req.body.email,htmlData)
            console.log("---sendOtp---->",sendOtp)

            let save = await service.createUser(req.body)
            console.log('-----save')
            ...
            return response(res, statusCode.data.SUCCESS, save, messages.SuccessMessage.OTP_SEND)
        } catch (error) {
     ...
     *     parameters:
     *       - name: logginId
     *         description: logginId can be email, phone number or username
     *         in: formData
     *         required: true
     *       - name: password
    ...
    login: async (req, res, next) => {
        try {
            let userFind = await service.findUser({$or:[{userName:req.body.logginId},{email:req.body.logginId},{phoneNumber:req.body.logginId}], userType: userTypeEnum.data.USER, status: { $ne: statusEnum.data.DELETED } })
            if (!userFind) {
            ...
            if(!userFind.otpVerified){
                req.body.otp = commonFunction.getOTP()
                let update=await service.updateUser({_id:userFind._id},{$set:{otp:req.body.otp}})
                return response(res, statusCode.data.REDIRECT, {otp:update.otp,_id:userFind._id,otpVerified:userFind.otpVerified}, messages.ErrorMessage.ACCOUNT_NOT_VERIFIED)
            }
            let passwordCheck=bcrypt.compareSync(req.body.password,userFind.password)
            if(!passwordCheck){
                return response(res, statusCode.data.INVALID_CREDENTIAL, {}, messages.ErrorMessage.WRONG_PASSWORD)
            }

            let token = jwt.sign({ _id: userFind._id }, "anime-centre299865", { expiresIn: '12h' })            
            return response(res, statusCode.data.SUCCESS, {data:userFind,token}, messages.SuccessMessage.LOGIN_SUCCESS)
    ...
    forgotPassword:async(req,res,next)=>{
        try {
            let userFind = await service.findUser({email:req.body.email, userType: userTypeEnum.data.USER, status: { $ne: statusEnum.data.DELETED } })
            if (!userFind) {
                return response(res, statusCode.data.NOT_FOUND, {}, messages.ErrorMessage.NOT_FOUND)
            }
            ...
            return response(res, statusCode.data.SUCCESS,
                {
                    _id:update._id,
                    otp:update.otp
                },
                messages.SuccessMessage.LOGIN_SUCCESS)
...
*/
    otpVerifyForPlatform: async (req, res, next) => {
        try {
            ...
            if ((userFind.otpExpireTime + 30 * 60 * 60 * 1000) < Date.now()) {
                return response(res, statusCode.data.FORBIDDEN, {}, messages.ErrorMessage.OTP_EXPIRED)
            }
            if (userFind.otp != req.body.otp) {
                return response(res, statusCode.data.FORBIDDEN, {}, "Incorrect otp.")
            }
            var token = jwt.sign({ _id: userFind._id, iat: Math.floor(Date.now() / 1000) - 30 }, 'anime-center', { expiresIn: '365d' });
